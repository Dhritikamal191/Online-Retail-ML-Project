...
class TrainModels:
    ...
    def train(self):
        ...
        cluster_profiles = (self.df.groupby("Cluster").mean(numeric_only=True).round(2))

        cluster_profiles['Segment'] = 'Customer Segment'

        sorted_clusters = cluster_profiles.sort_values(by='Monetary',ascending=False).index.tolist()
